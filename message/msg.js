...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const isOwner = ownerNumber == sender ? true : ["6281249086533@s.whatsapp.net", "6281249086533@s.whatsapp.net"].includes(sender) ? true : false
		...
		const buttonsDefault = [
			{ callButton: { displayText: `Call Owner!`, phoneNumber: `+6281249086533` } },
			{ urlButton: { displayText: `Script!`, url : `bentar msi w update` } },
		...
		switch(command) {
			...
			case prefix+'help':
			    ...
			    conn.sendMessage(from, { caption: teks, location: { jpegThumbnail: fs.readFileSync(setting.pathimg) }, templateButtons: buttonsDefault, footer: 'Jem-Bot © 2022', mentions: [sender] })
				break
			case prefix+'runtime':
